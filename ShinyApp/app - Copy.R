    ...
    dashboardBody(align="center",
        ...
        column(12,
               box(
                   ...
                   tags$h5("This analysis uses a Goodness of Fit test on the COVID-19 testing data by students in DATA2X02. This is done by using a Chi-squared approximation. Continuous distributions are continuity corrected and binned into integer positions using their cumulative distributive functions."),
  ...
  #renderUI
    ...
    output$plot1 <- renderPlotly({
        ...
        if(input$dist_input == "Exponential"){ 
          ...
          output$description = renderUI({
            str1 <- withMathJax(paste("\\(H_0\\): ","The data came from a ", "\\(\\text{Exp}(\\lambda)\\)", "distribution with parameter ", "\\(\\lambda = ",input$dist_param, "\\)."));
            str2 <- withMathJax(paste("\\(H_A\\): ","The data did not come from a ", "\\(\\text{Exp}(\\lambda)\\)", "distribution with parameter ", "\\(\\lambda = ",input$dist_param, "\\)."));
            HTML(paste(str1,str2, sep = '<br/>'))
          })
